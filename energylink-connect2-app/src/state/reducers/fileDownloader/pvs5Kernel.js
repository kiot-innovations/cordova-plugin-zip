...
export default createReducer(
  {
    [PVS5_SET_FW_INFO]: (
      ...
      const { status } = state
      const needToDownloadKernel =
        status === 'idle' ||
        status === 'error' ||
        (status === 'downloaded' && shouldRetry)

      if (needToDownloadKernel) {
        return {
          ...state,
          name: versionName,
          updateURL,
    ...
    [PVS5_KERNEL_DOWNLOAD_PROGRESS]: (state, { progress, size, step }) => {
      if (state.status === 'downloading')
        return { ...state, size, step, progress }
      return state
    },

    [PVS5_KERNEL_VERIFY_DOWNLOAD]: (state, payload) => {
      if (state.status === 'downloading')
        return { ...state, status: 'verifying' }
      return state
    },

    [PVS5_KERNEL_DOWNLOAD_SUCCESS]: (state, { lastModified, size }) => {
      if (state.status === 'verifying')
        return { ...state, status: 'downloaded', lastModified, size }
      return state
    },

    [PVS5_KERNEL_DOWNLOAD_ERROR]: (state, error) => ({
      ...state,
      error,
      status: 'error'
    })
