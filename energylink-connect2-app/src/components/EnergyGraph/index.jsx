...
const XTick = ({ view, data, width, height, x, y, payload }) => {
  ...
  const angle = VIEW_OPTIONS[view].X_LABELS_ANGLE || 0
  const yTranslate = angle !== 0 ? 10 : 0
  ...
  return (
    <g>
      <text
        ...
        x={x}
        y={y}
        transform={`translate(0, ${yTranslate}) rotate(${angle}, ${x}, ${y})`}
        stroke="none"
        fill="#F8F8F8"
        fontSize="10"
        textAnchor="middle"
...
function EnergyGraph({
  ...
  let min = hasStorage
    ? data.reduce(
        (acc, curr) =>
          Math.min(curr[series[0]], curr[series[1]], curr[series[2]], acc),
        0
      )
    : 0

  min = min < -1 ? Math.floor(min) : min
  min = min > -yMargin ? (min >= 0 ? 0 : -yMargin) : min

  const interval =
    viewOptions.X_INTERVAL === null
      ? Math.floor(data.length / 8)
      : viewOptions.X_INTERVAL
